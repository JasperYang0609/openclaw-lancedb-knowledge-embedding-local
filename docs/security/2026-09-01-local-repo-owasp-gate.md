...
- A03 PASS — immutable Qwen and llama.cpp revisions, byte counts, SHA-256, original and redirect host allowlists, exact runtime build/commit/platform verification, immutable GitHub Action pins and license inventory; official artifacts passed live verification and the distributed archive includes the MIT license.
...
- A10 PASS — partial download retention, live official-artifact resume, range fallback, size ceiling, checksum/redirect failure, stale PID, port collision, orphan-listener uninstall refusal, partial staging and cleanup failure behavior.

ASVS: NOT_APPLICABLE_WITH_EVIDENCE. This is a local CLI and loopback sidecar, not a public Web/API authentication surface.

Attacker review: path injection, nested model symlink planting, archive traversal/escaping or cyclic links/devices, manifest forgery, PID reuse/loss, download tamper or redirect, unsafe uninstall and cross-provider configuration are fail-closed. The 2.69 GiB model remains outside Git; live verification used an existing hash-matched artifact. Two fresh install/uninstall rounds, official runtime resume, vector parity, 20-query quality and 20-query p95 gates passed without modifying Production Gemini.
