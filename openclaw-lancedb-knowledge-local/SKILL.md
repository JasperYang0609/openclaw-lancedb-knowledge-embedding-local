...
The OpenClaw integration is also an idempotent upgrader. It inventories all cron jobs with completeness checks, proves atomic no-replace publication before cron/runtime mutation, changes only exact installer-owned declarations, leaves unknown and Gemini rollback assets untouched, and enables the two recurring jobs only after both disabled contracts pass global readback. A fresh custom snapshot path may have a missing parent: the installer probes its nearest existing safe ancestor, then creates and receipts the managed root without following symlinks. A failed mutation restores the prior owned files and full job definitions before reporting failure. Recovery accepts destructive cleanup only for current receipts whose canonical roots and exact inode ownership are present. Older public contract-v2 roots without inode fields are preserved and reported; contract-v1 recovery fails closed for manual review.

Unknown jobs that target an owned wrapper fail closed. The only exception is an independently reviewed, exact pre-declaration incremental job that is already disabled. Use the current-version diagnostic's job ID and **ID-inclusive** normalized-contract SHA-256 together with `--approve-disabled-collision-role incremental`; all three options are mandatory. Never calculate or reuse an ID-excluding hash. The exception is unavailable for snapshot jobs or look-alikes, leaves the approved job customer-owned and byte-for-byte unchanged, and is reused only from a private committed transaction receipt. Any identity, enabled-state, declaration-key, argv, schedule, delivery, alert, limit, environment, tools, or extra-field drift blocks before mutation.
